...
class LightweightEncoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            ...
            outputs.append(out)
        return outputs  # Return list of features per layer
...
class UNetStyleOriginalEncoder(nn.Module):
    def __init__(self, num_layers, input_dim, num_f_maps, high_encoder_depth):
        super(UNetStyleOriginalEncoder, self).__init__()
        self.input_proj = nn.Conv1d(input_dim, num_f_maps, 1)
        self.layers = nn.ModuleList([
        ...
        self.cross_attention_layers = nn.ModuleList([
            AttLayer(
                q_dim=num_f_maps, k_dim=num_f_maps, v_dim=num_f_maps,
                r1=2, r2=2, r3=2, bl=2 ** i, stage='encoder', att_type='normal_att'
            ) for i in range(high_encoder_depth)
        ])
        self.high_encoder_depth = high_encoder_depth

    def forward(self, x_low, high_features, mask):
        out = self.input_proj(x_low)
        fused_features = []

        high_idx = self.high_encoder_depth - 1  # reverse order
        for i, layer in enumerate(self.layers):
            if high_idx >= 0:
                high_feat = high_features[high_idx]
                out = self.cross_attention_layers[high_idx](out, high_feat, mask)
                high_idx -= 1
            out = layer(out, None, mask)
...
class DualEncoderASFormer(nn.Module):
    ...
    def forward(self, x_low, x_high, mask):
        ...
        encoded_high_list = self.encoder_high(x_high, mask)  # list of features: high_1, ..., high_N
        feat_low = self.encoder_low(x_low, encoded_high_list, mask)  # fused via cross-attention

        # Predict directly from the final low feature
        scores = self.fusion_predictor(feat_low, mask)
